...
- [ ] Admin can block/unblock identities with confirmation, reasons where required, and audit evidence.
- [ ] Admin can view and update Tier 1 caps using exact decimal-string values and server-side policy validation.
- [ ] Admin can view and operate pilot controls, engineering isolation, allowlists, exposure policy, readiness stages, approvals, and publication according to authority rules.
- [ ] Actions are idempotent where consequential, distinguish queued from completed, and expose safe audit/correlation references.
