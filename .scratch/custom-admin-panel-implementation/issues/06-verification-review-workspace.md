...
- [x] Verification list/detail read models expose the review state and evidence needed by the permitted operator without leaking restricted data.
- [x] Approved, rejected, and escalated decisions require a decision reason and are validated server-side.
- [x] Reviewer independence and role policy are enforced by the backend, not only by UI visibility.
- [x] Each decision records immutable audit evidence and safe result/correlation references.
