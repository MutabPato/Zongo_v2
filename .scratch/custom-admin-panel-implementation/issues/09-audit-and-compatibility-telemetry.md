...
- [ ] Audit list/detail read models provide role-filtered, paginated, append-only visibility.
- [ ] Audit records expose safe actor, role, target, reason, timestamp, and correlation references without editable controls.
- [ ] Every legacy `/admin` alias is mapped to a `/admin/v1` operation and has usage/deprecation telemetry and an owner record.
- [ ] Browser routes never depend on bearer compatibility, while named automation/tests remain covered.
